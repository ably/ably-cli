...
async function _waitForStringInOutput(
  ...
  throw new Error(
    `Timeout waiting for string "${targetString}" in output after ${timeoutMs}ms. Recent output:\n${recentOutput}`,
  );
}
  ...
  describe("Room occupancy functionality", function () {
    it("should show occupancy metrics for active room", async function () {
      ...
      try {
        ...
        expect(
          occupancyResult!.exitCode,
          `Exit code should be 0. stderr: ${occupancyResult!.stderr}`,
        ).to.equal(0);
  ...
  describe("Message publish and subscribe functionality", function () {
    it("should allow publishing and subscribing to messages", async function () {
      ...
      try {
        ...
        expect(
          isSuccessful,
          `Message should be sent successfully. Exit code: ${sendResult.exitCode}, stdout: ${sendResult.stdout}, stderr: ${sendResult.stderr}`,
        ).to.be.true;
        ...
        expect(
          isSecondSuccessful,
          `Second message should be sent successfully. Exit code: ${sendResult2.exitCode}, stdout: ${sendResult2.stdout}, stderr: ${sendResult2.stderr}`,
        ).to.be.true;
