...
describe("E2E: ably bench publisher and subscriber", function () {
  ...
  it("should run publisher and subscriber, and report correct message counts", async function () {
      ...
      const subscriberPromise = new Promise<void>(
        (resolveSubscriber, rejectSubscriber) => {
          ...
          subscriberProcess.stdout.on("data", (data) => {
            ...
            if (DEBUG_OUTPUT)
              process.stdout.write(`[DEBUG_SUB_OUT] ${outputChunk}`); // Pipe to main stdout only if debugging
      ...
      const publisherPromise = new Promise<void>(
        (resolvePublisher, rejectPublisher) => {
          ...
          publisherProcess.stdout.on("data", (data) => {
            const outputChunk = data.toString();
            if (DEBUG_OUTPUT)
              process.stdout.write(`[DEBUG_PUB_OUT] ${outputChunk}`); // Pipe to main stdout only if debugging
    ...
    expect(
      publisherSummary,
      "Publisher summary not found or not in JSON format",
    ).to.exist;
    expect(publisherSummary?.data).to.exist;
    if (publisherSummary?.data) {
      expect(
        publisherSummary.data.messagesSent,
        "Publisher messagesSent mismatch",
      ).to.equal(messageCount);
      expect(
        publisherSummary.data.messagesEchoed,
        "Publisher messagesEchoed seems too low",
      ).to.be.greaterThanOrEqual(messageCount * 0.9);
      expect(
        publisherSummary.data.errors,
        "Publisher errors should be 0",
      ).to.equal(0);
    }
    ...
    expect(
      subscriberSummary,
      "Subscriber summary not found or not in JSON format",
    ).to.exist;
    expect(subscriberSummary?.data?.results).to.exist;
    if (subscriberSummary?.data?.results) {
      expect(
        subscriberSummary.data.results.messagesReceived,
        "Subscriber messagesReceived mismatch",
      ).to.equal(messageCount);
    }
  });
});
