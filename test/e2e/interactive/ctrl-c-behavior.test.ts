...
describe("E2E: Interactive Mode - Ctrl+C Behavior", () => {
    ...
    it(
      ...
      async () => {
        ...
        expect(output).toMatch(
          /Signal received|Type 'exit' to quit|To exit.*type 'exit'/i,
        );

        // Should still have prompt
        expect(output).toContain("ably>");
      ...
      async () => {
        ...
        await new Promise<void>((resolve) => {
          const checkInterval = setInterval(() => {
            ...
            const promptCount = lines.filter((line) =>
              line.includes("ably> "),
            ).length;
            if (promptCount >= 2) {
              // Initial prompt + prompt after interrupt
              clearInterval(checkInterval);
              resolve();
            }
          }, 100);
      ...
      async () => {
        ...
        proc = spawn("node", [cliPath, "interactive"], {
          ...
          env: {
            ...process.env,
            ABLY_API_KEY:
              process.env.E2E_ABLY_API_KEY || process.env.ABLY_API_KEY,
            ABLY_SUPPRESS_WELCOME: "1",
          },
        });
        ...
        expect(allOutput).toContain("↓ Stopping");
        ...
        expect(promptCount).toBeGreaterThanOrEqual(2); // Initial prompt + prompt after interrupt
  ...
  describe("Edge Cases", () => {
    it(
      ...
      async () => {
        ...
        expect(helpOutput).toMatch(/help|Usage:|Commands:/i);
    ...
    it(
      ...
      async () => {
        ...
        expect(output).toContain("still working after 5 Ctrl+C");
  ...
  describe("Real TTY Behavior", () => {
    it(
      ...
      async () => {
        ...
        proc = spawn(
          "node",
          [path.join(__dirname, "../../../bin/run.js"), "interactive"],
          {
            stdio: ["pipe", "pipe", "pipe"],
            env: { ...process.env, ABLY_SUPPRESS_WELCOME: "1" },
          },
        ...
        expect(output).toContain("help"); // Help output
    ...
    it(
      ...
      async () => {
        ...
        expect(output).not.toContain("Failed to start interactive mode");
