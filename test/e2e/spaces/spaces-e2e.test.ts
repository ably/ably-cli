...
describe("Spaces E2E Tests", function () {
  ...
  describe("Members presence functionality", function () {
    it("should allow two connections where one person entering is visible to the other", async function () {
      ...
      try {
        ...
        expect(memberUpdateReceived, "Client1 should see client2's space entry")
          .to.be.true;
  ...
  describe("Location state synchronization", function () {
    it("should synchronize location updates between clients", async function () {
      ...
      try {
        ...
        expect(
          isLocationSetSuccessful,
          `Location should be set successfully. Exit code: ${setLocationResult.exitCode}, stdout: ${setLocationResult.stdout}, stderr: ${setLocationResult.stderr}`,
        ).to.be.true;
        expect(setLocationResult.stdout).to.contain(
        ...
        expect(
          ...
          "Client1 should receive location update from client2",
        ).to.be.true;
        ...
        expect(
          isSecondLocationSetSuccessful,
          `Second location should be set successfully. Exit code: ${updateLocationResult.exitCode}, stdout: ${updateLocationResult.stdout}, stderr: ${updateLocationResult.stderr}`,
        ).to.be.true;
        ...
        expect(
          secondLocationUpdateReceived,
          "Client1 should receive second location update",
        ).to.be.true;
      } finally {
        if (locationsProcess) {
    ...
    it("should synchronize cursor updates between clients", async function () {
      ...
      try {
        ...
        expect(
          ...
          "Client1 should receive cursor update from client2",
        ).to.be.true;
        ...
        expect(
          getAllResult.exitCode,
          `getAll should succeed. Exit code: ${getAllResult.exitCode}, stdout: ${getAllResult.stdout}, stderr: ${getAllResult.stderr}`,
        ).to.equal(0);
  ...
  describe("Locks synchronization", function () {
    it("should synchronize lock acquisition and release between clients", async function () {
      ...
      try {
        ...
        expect(
          lockAcquiredReceived,
          "Client1 should receive lock acquisition from client2",
        ).to.be.true;
        ...
        expect(
          lockReleasedReceived,
          "Client1 should receive lock release notification",
        ).to.be.true;
      } finally {
        if (locksProcess) {
  ...
  describe("Command Structure Tests", function () {
    it("should have properly structured spaces member commands", async function () {
      ...
      expect(helpResult.stdout).to.contain(
