...
if (!process.env.CI || process.env.VERBOSE_TESTS) {
  console.log(
    `[Playwright Config] Loading configuration at ${new Date().toISOString()}`,
  );
  ...
  console.log(
    `[Playwright Config] Environment: ${process.env.CI ? "CI" : "Local"}`,
  );
}
...
if (!process.env.CI || process.env.VERBOSE_TESTS) {
  ...
  console.log(
    `[Playwright Config] Configuration complete at ${new Date().toISOString()}`,
  );
}
