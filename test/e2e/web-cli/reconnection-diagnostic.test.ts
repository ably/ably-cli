...
test.describe("Web CLI Reconnection Diagnostic E2E Tests", () => {
  ...
  test("exposes correct debugging information", async ({ page }) => {
    ...
    await page.goto(
      `${getTestUrl()}?serverUrl=${encodeURIComponent(TERMINAL_SERVER_URL)}&cliDebug=true`,
    );
  ...
  test.skip("debugging functions persist through reconnection", async ({
    page,
  }) => {
    ...
    expect(postReconnectDebugInfo.reactState.componentConnectionStatus).toBe(
      "connected",
    );
