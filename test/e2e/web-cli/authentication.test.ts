...
test.describe("Web CLI Authentication E2E Tests", () => {
  ...
  test("should validate API key is required", async ({ page }) => {
    ...
    await expect(
      page.getByText("API Key is required to connect to Ably"),
    ).toBeVisible();
  ...
  test("should validate API key format", async ({ page }) => {
    ...
    await page.fill(
      'input[placeholder="your_app.key_name:key_secret"]',
      "invalid-format",
    );
    await page.click('button:has-text("Connect to Terminal")');
    ...
    await expect(
      page.getByText(
        "API Key should be in the format: app_name.key_name:key_secret",
      ),
    ).toBeVisible();
  ...
  test("should authenticate with valid API key and show terminal", async ({
    page,
  }) => {
    // Wait for any ongoing rate limit pause
    await waitForRateLimitLock();
  ...
  test("should persist authentication state across page reloads with remember checked", async ({
    page,
  }) => {
    // Wait for any ongoing rate limit pause
    await waitForRateLimitLock();
  ...
  test("should not persist authentication state across page reloads when remember is unchecked", async ({
    page,
  }) => {
    // Wait for any ongoing rate limit pause
    await waitForRateLimitLock();
  ...
  test("should allow changing credentials via auth settings", async ({
    page,
  }) => {
    // Wait for any ongoing rate limit pause
    await waitForRateLimitLock();
  ...
  test("should show credential display with proper redaction", async ({
    page,
  }) => {
    // Wait for any ongoing rate limit pause
    await waitForRateLimitLock();
  ...
  test.skip("should handle authentication with access token", async ({
    ...
    await page.fill(
      'input[placeholder="Your JWT access token"]',
      "test-access-token",
    );
    incrementConnectionCount();
    await page.click('button:has-text("Connect to Terminal")');
  ...
  test("should maintain terminal session when updating auth settings without changing credentials", async ({
    page,
  }) => {
    // Wait for any ongoing rate limit pause
    await waitForRateLimitLock();
  ...
  test.skip("should show SERVER DISCONNECT overlay for invalid credentials", async ({
    ...
    await Promise.race([
      ...
      expect(terminalElement).toContainText(/error|Error|failed|Failed/, {
        timeout: 10000,
      }),
...
test.describe("Web CLI Auto-Login E2E Tests", () => {
  ...
  test("should automatically authenticate when API key is provided via query parameter", async ({
    page,
  }) => {
    // Wait for any ongoing rate limit pause
    await waitForRateLimitLock();
  ...
  test("should allow switching from query param auth to custom auth", async ({
    page,
  }) => {
    // Wait for any ongoing rate limit pause
    await waitForRateLimitLock();
