...
test.describe("Web CLI Reconnection E2E Tests", () => {
  ...
  test("should handle disconnection and reconnection gracefully", async ({
    page,
  }) => {
    ...
    log(
      `Waiting ${rateDelay / 1000} seconds before test to avoid rate limits...`,
    );
    ...
    if (requiresManualReconnect) {
      throw new Error(
        "Test failed: Terminal requires manual reconnection (Enter key) instead of automatically reconnecting. This is a bug!",
      );
    }
  ...
  test("should show reconnection status messages", async ({ page }) => {
    ...
    const statusVisible = await Promise.race([
      page
        ...
        .then(() => "disconnected"),
      page
        .locator(".App-header span.status-reconnecting")
        .waitFor({ state: "visible", timeout: 5000 })
        .then(() => "reconnecting"),
    ]).catch(() => null);
    ...
    if (statusVisible === "disconnected") {
      await expect(
        page.locator(".App-header span.status-reconnecting"),
    ...
    await expect(page.locator(".App-header span.status-connected")).toBeVisible(
      { timeout: 5000 },
    );
    await expect(page.locator(".App-header span.status-connected")).toHaveText(
      "connected",
    );
  });
  ...
  test("should allow cancelling auto-reconnect via Enter key", async ({
    page,
  }) => {
    // Wait for any ongoing rate limit pause
    await waitForRateLimitLock();
    ...
    await expect(page.locator(statusSelector)).toHaveText("connected", {
      timeout: 15000,
    });
    ...
    try {
      await expect(page.locator(terminalSelector)).toContainText(
        /Reconnecting|reconnecting|disconnect|connection/i,
        { timeout: 5000 },
      );
    } catch (_error) {
      // If no reconnection message visible, the connection may have recovered too quickly
      console.log(
        "No reconnection UI visible - connection may have recovered immediately",
      );
      return;
    }
    ...
    await expect(page.locator(statusSelector)).toHaveText("disconnected", {
      timeout: 5000,
    });
    await expect(page.locator(terminalSelector)).toContainText(
    ...
    await page.keyboard.press("Enter");
    await expect(page.locator(statusSelector)).toHaveText("connecting", {
      timeout: 5000,
    });
    await expect(page.locator(statusSelector)).toHaveText("connected", {
      timeout: 15000,
  ...
  test.skip("should show manual reconnect prompt after max attempts - COMPLEX test with timing issues", async ({
    ...
    await expect(page.locator(terminalSelector)).toContainText(
      "Press Enter to try reconnecting manually",
      { timeout: 5000 },
    );
