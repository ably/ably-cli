...
test.describe("Session Resume E2E Tests", () => {
  ...
  test("connects to public server and can resume session after reconnection", async ({
    page,
  }) => {
    ...
    await page.goto(
      `${getTestUrl()}?serverUrl=${encodeURIComponent(TERMINAL_SERVER_URL)}`,
    ...
    while (
      ...
      if (
        currentState.componentStatus === "connected" &&
        currentState.isSessionActive &&
        currentState.socketExists &&
        currentState.socketReadyState === 1
      ) {
        // WebSocket.OPEN
        reconnectionComplete = true;
        log("Reconnection complete - connection is stable");
      } else if (
        currentState.componentStatus === "reconnecting" ||
    ...
    if (!reconnectionComplete) {
      ...
      throw new Error(
        `Reconnection did not complete within timeout. Final state: ${JSON.stringify(finalState)}`,
      );
    }
    ...
    if (requiresManualReconnect) {
      throw new Error(
        "Test failed: Terminal requires manual reconnection instead of automatically reconnecting for session resume",
      );
    }
    ...
    if (!newSocketInfo.exists || newSocketInfo.readyState !== 1) {
      throw new Error(
        `Expected socket to be open after reconnection, but got: ${JSON.stringify(newSocketInfo)}`,
      );
    }
  ...
  test("preserves session across page reload when resumeOnReload is enabled", async ({
    page,
  }) => {
    // Wait for any ongoing rate limit pause
    await waitForRateLimitLock();
    ...
    for (let i = 0; i < 1; i++) {
      ...
      const preReloadState = await page.evaluate(() => {
        ...
        return {
          ...
          sessionStorage: {
            ...
            credentialHash: sessionStorage.getItem(
              `ably.cli.credentialHash.${domain}`,
            ),
          },
          localStorage: {
      ...
      log(
        "Post-reload storage state:",
        JSON.stringify(postReloadState, null, 2),
      );
