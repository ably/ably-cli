...
async function globalSetup() {
  ...
  if (!process.env.CI || process.env.VERBOSE_TESTS) {
    ...
    console.log(
      `[Global Setup] Memory usage: ${JSON.stringify(process.memoryUsage())}`,
    );
  ...
  if (existsSync(envPath)) {
    ...
    if (result.error) {
      console.warn(
        `[Global Setup] Warning: Error loading .env file: ${result.error.message}`,
      );
    } else if (
      result.parsed &&
    ...
    console.log(
      "[Global Setup] No .env file found. Using environment variables from current environment.",
    );
  }
      ...
      await new Promise<void>((resolve, reject) => {
        https
          .get(testUrl.href, (res) => {
            console.log(
              `[Global Setup] HTTPS connection test: status ${res.statusCode}`,
            );
            res.destroy();
            resolve();
          })
  ...
  if (!process.env.CI || process.env.VERBOSE_TESTS) {
    console.log(
      `[Global Setup] Configuring rate limiter at ${new Date().toISOString()}`,
    );
  }
  setupRateLimiter();
  ...
  if (!process.env.SKIP_INITIAL_DELAY && !process.env.CI_BYPASS_SECRET) {
    ...
    if (!process.env.CI || process.env.VERBOSE_TESTS) {
      console.log(
        `[Global Setup] Waiting ${initialDelay / 1000} seconds to ensure clean rate limit window...`,
      );
    }
  ...
  if (!process.env.CI || process.env.VERBOSE_TESTS) {
    console.log(
      `[Global Setup] Starting web server at ${new Date().toISOString()}`,
    );
  }
  ...
  if (!process.env.CI || process.env.VERBOSE_TESTS) {
    console.log(
      `[Global Setup] Web server ready at ${url} at ${new Date().toISOString()}`,
    );
  }
  ...
  if (!process.env.CI || process.env.VERBOSE_TESTS) {
    ...
    console.log(
      `[Global Setup] Total setup duration: ${Date.now() - startTime}ms`,
    );
  }
  ...
  return async () => {
    if (!process.env.CI || process.env.VERBOSE_TESTS) {
      console.log(
        `[Global Setup] Cleanup called at ${new Date().toISOString()}`,
      );
    }
  };
}
