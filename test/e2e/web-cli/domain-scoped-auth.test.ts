...
test.describe("Domain-Scoped Authentication E2E Tests", () => {
  ...
  test("should store credentials scoped to WebSocket domain", async ({
    page,
  }) => {
    ...
    expect(
      storedKeys.some(
        (key) =>
          key.includes(".rememberCredentials.") && key.includes(expectedDomain),
      ),
    ).toBe(true);
  });

  test("should not share credentials between different domains", async ({
    page,
  }) => {
    // Wait for any ongoing rate limit pause
    await waitForRateLimitLock();
    ...
    const exampleKey = Object.entries(credentialData).find(([k]) =>
      k.includes("example.com"),
    )?.[1];
  ...
  test("should clear only current domain credentials when clearing", async ({
    page,
  }) => {
    // Wait for any ongoing rate limit pause
    await waitForRateLimitLock();
    ...
    const otherDomainKeys = Object.keys(remainingCredentials).filter((k) =>
      k.includes("other-domain.com"),
    );
  ...
  test.skip("should use correct domain-scoped credentials when serverUrl parameter changes", async ({
    page,
  }) => {
    ...
    const maliciousUrl = getTestUrl().includes("?")
      ? getTestUrl() +
        "&serverUrl=wss://malicious.example.com&clearCredentials=true"
      : getTestUrl() +
        "?serverUrl=wss://malicious.example.com&clearCredentials=true";
