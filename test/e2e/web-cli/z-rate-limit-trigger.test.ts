...
test.describe("Z-Rate Limit Config Test - MUST RUN LAST", () => {
  ...
  test("should handle server disconnections and verify reconnection configuration", async ({
    page,
  }) => {
    ...
    if (
      ...
      process.env.RATE_LIMIT_CONFIG === "CI_EMERGENCY"
    ) {
    ...
    log(
      `Current rate limiter state: ${rateLimitState.connectionCount} connections`,
    );
    ...
    if (isLikelyRateLimited) {
      log(
        `High connection count (${rateLimitState.connectionCount}) - expecting rate limiting behavior`,
      );
    }
    ...
    if (!apiKey) {
      throw new Error(
        "E2E_ABLY_API_KEY or ABLY_API_KEY environment variable is required",
      );
    }
    ...
    const hasCode4000Error = await page
      .getByText(/Connection closed by server \(4000\)/i)
      .isVisible({ timeout: 1000 })
      .catch(() => false);
    let hasRateLimitError = await page
      .getByText(/429|rate limit/i)
      .isVisible({ timeout: 1000 })
      .catch(() => false);
    ...
    if (
      (isCI && connectionState.isReconnecting) ||
      (isLikelyRateLimited && connectionState.isReconnecting)
    ) {
      log(
    ...
    if (hasCode4000Error) {
      ...
      const hasDisconnectUI = await page
        .getByText(/ERROR: SERVER DISCONNECT/i)
        .isVisible({ timeout: 5000 })
        .catch(() => false);
      const hasManualReconnectPrompt = await page
        .getByText(/Press = to try reconnecting manually/i)
        .isVisible({ timeout: 5000 })
        .catch(() => false);
      ...
      // Verify status
      const disconnectedStatus = await page
        .locator(".App-header span.status-disconnected")
        .isVisible({ timeout: 5000 })
        .catch(() => false);
    ...
    if (hasRateLimitError) {
      ...
      log(
        `Using ${waitTimeout}ms timeout for rate limit scenario (CI: ${isCI})`,
      );
      ...
      if (maxReached) {
        ...
        if (isCI && finalState?.componentConnectionStatus === "reconnecting") {
          ...
          expect(["connected", "reconnecting", "disconnected"]).toContain(
            finalState?.componentConnectionStatus,
          );
        }
      }
    ...
    if (state.status === "connected") {
      ...
      log(
        `✓ Reconnection configuration verified (max attempts: ${expectedMaxAttempts})`,
      );

      // Verify UI is in proper state
      const connectedStatus = await page
        .locator(".App-header span.status-connected")
        .isVisible({ timeout: 5000 })
        .catch(() => false);
      expect(connectedStatus).toBe(true);
    } else if (state.status === "reconnecting") {
      log(
        "Component is reconnecting - this is expected behavior in high-load scenarios",
      );
      expect(["reconnecting", "connecting"]).toContain(state.status);
    } else {
      // Handle other states that might occur under load
      log(
        `Component in state: ${state.status} - verifying this is a valid state`,
      );
      expect([
        "connected",
        "reconnecting",
