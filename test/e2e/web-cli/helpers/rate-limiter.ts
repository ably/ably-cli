...
class GlobalRateLimiter {
  ...
  private getRecentAttempts(): ConnectionAttempt[] {
    ...
    return this.connectionAttempts.filter(
  ...
  private recordAttempt(testName: string, success: boolean): void {
    ...
    console.log(
      `[RateLimiter] Recorded connection attempt at ${new Date().toISOString()}`,
    );
    console.log(
      `[RateLimiter] Test: "${testName}", Success: ${success}, Process ID: ${process.pid}`,
    );
    console.log(
      `[RateLimiter] Total attempts in window: ${this.getRecentAttempts().length}`,
    );
  }
  ...
  async waitForRateLimit(testName: string): Promise<void> {
    ...
    console.log(
      `[RateLimiter] Test "${testName}" waiting for rate limit clearance...`,
    );

    const recentAttempts = this.getRecentAttempts();
    console.log(
      `[RateLimiter] Current connections in window: ${recentAttempts.length}/${this.config.maxConnectionsPerMinute}`,
    );
  ...
  async executeWithRateLimit<T>(
    ...
    console.log(
      `[RateLimiter] executeWithRateLimit called for "${testName}" at ${new Date().toISOString()}`,
    );
    ...
    for (let attempt = 0; attempt < this.config.maxRetries; attempt++) {
      try {
        ...
        console.log(
          `[RateLimiter] Test "${testName}" attempting connection (attempt ${attempt + 1}/${this.config.maxRetries}) at ${new Date().toISOString()}`,
        );
        ...
        console.log(
          `[RateLimiter] Test "${testName}" connected successfully at ${new Date().toISOString()}`,
        );
        ...
        console.error(
          `[RateLimiter] Test "${testName}" connection failed:`,
          error,
        );

        if (attempt < this.config.maxRetries - 1) {
          console.log(
            `[RateLimiter] Retrying in ${this.config.retryDelayMs}ms...`,
          );
...
export async function navigateWithRateLimit(
  ...
  return globalRateLimiter.executeWithRateLimit(testName, async () => {
    await page.goto(url);
...
export async function waitForRateLimitWindow(): Promise<void> {
  ...
  if (status.recentAttempts > 0) {
    console.log(
      `[RateLimiter] Waiting for rate limit window to clear (${status.recentAttempts} recent attempts)`,
    );
