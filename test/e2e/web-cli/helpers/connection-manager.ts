...
export async function establishConnection(
  ...
  console.log(
    `[ConnectionManager] establishConnection called at ${new Date().toISOString()}`,
  );
  console.log(
    `[ConnectionManager] Test name: "${testName}", Process ID: ${process.pid}`,
  );
  console.log(`[ConnectionManager] Server URL: ${serverUrl || "default"}`);

  // Log stack trace to understand call context (only if verbose logging is enabled)
  if (process.env.VERBOSE_LOGGING === "true") {
    const stack = new Error("Stack trace for connection establishment").stack;
    console.log(
      `[ConnectionManager] Establish connection stack trace:\n${stack}`,
    );
  }
  ...
  await connectWithRateLimit(
    ...
    async () => {
      ...
      const url =
        serverUrl || process.env.WEB_CLI_TEST_URL || "http://localhost:5173";
      console.log(
        `[ConnectionManager] About to navigate to: ${url} at ${new Date().toISOString()}`,
      );
      console.log(
        `[ConnectionManager] This will create a new WebSocket connection`,
      );

      await page.goto(url);
      console.log(
        `[ConnectionManager] Navigation complete, WebSocket connection should be initiated`,
      );
      ...
      console.log(
        `[ConnectionManager] Waiting for terminal selector: ${terminalSelector}`,
      );
  ...
  console.log(
    `[ConnectionManager] Connection established successfully for test: "${testName}" at ${new Date().toISOString()}`,
  );
}
 ...
 */
export async function waitForConnectionStable(
  page: Page,
  timeout = 5000,
): Promise<void> {
...
export async function executeCommandWithRetry(
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      if (status !== "connected") {
        console.log(
          `[ConnectionManager] Not connected (${status}), waiting...`,
        );
      ...
      console.error(
        `[ConnectionManager] Command execution failed (attempt ${attempt + 1}/${maxRetries}):`,
        error,
      );
      ...
      const terminalContent = await page
        ...
        .textContent();
      console.error(
        `[ConnectionManager] Terminal content: ${terminalContent?.slice(-500)}`,
      ); // Last 500 chars
  ...
  const monitor = async () => {
    while (monitoring) {
      try {
        ...
        console.log(
          `[ConnectionHealth] Status: ${health.componentStatus}, Session: ${health.isSessionActive ? "active" : "inactive"}, Socket: ${health.socketReadyState}`,
        );
      } catch (error) {
