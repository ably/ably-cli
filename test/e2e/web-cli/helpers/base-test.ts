...
export const test = base.extend({
  ...
  page: async ({ page }, use, testInfo) => {
    if (!process.env.CI || process.env.VERBOSE_TESTS) {
      console.log(
        `[BaseTest] Page fixture setup for "${testInfo.title}" at ${new Date().toISOString()}`,
      );
      console.log(
        `[BaseTest] Worker index: ${testInfo.workerIndex}, Process ID: ${process.pid}`,
      );
    }
    ...
    if (!process.env.CI || process.env.VERBOSE_TESTS) {
      page.on("websocket", (ws) => {
        ...
        console.log(
          `[WebSocket] Test: "${testInfo.title}", Process ID: ${process.pid}`,
        );
        ...
        ws.on("close", () => {
          console.log(
            `[WebSocket] Closed at ${new Date().toISOString()} (was created at ${wsCreatedAt})`,
          );
        });
    ...
    if (!process.env.CI || process.env.VERBOSE_TESTS) {
      console.log(
        `[BaseTest] Handing page to test at ${new Date().toISOString()}`,
      );
    }
    ...
    if (!process.env.CI || process.env.VERBOSE_TESTS) {
      console.log(
        `[BaseTest] Test finished using page at ${new Date().toISOString()}`,
      );
    }
    ...
    if (!process.env.CI || process.env.VERBOSE_TESTS) {
      console.log(
        `[BaseTest] Page fixture cleanup complete for "${testInfo.title}" at ${new Date().toISOString()}`,
      );
    }
  },
});
