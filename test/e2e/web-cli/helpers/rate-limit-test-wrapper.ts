...
export const test = base.extend({
  ...
  page: async ({ page }, use, testInfo) => {
    console.log(
      `[RateLimitWrapper] Test starting: "${testInfo.title}" at ${new Date().toISOString()}`,
    );
    ...
    console.log(
      `[RateLimitWrapper] Rate limit check complete, proceeding with test`,
    );
    ...
    console.log(
      `[RateLimitWrapper] Test finished: "${testInfo.title}" at ${new Date().toISOString()}`,
    );
  },
});
