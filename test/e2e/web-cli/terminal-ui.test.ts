...
test.describe("Web CLI Terminal UI Tests", () => {
  ...
  test.describe("Connection Animation", () => {
    test.skip("should display ASCII box animation during connection - TIMING sensitive test", async ({
      ...
      const _terminalContent = await page
        .locator(terminalRowsSelector)
        .textContent();
      ...
      if (statusText === "connected") {
        ...
        await expect(page.locator(terminalRowsSelector)).toContainText(
          "CONNECTING",
          { timeout: 5000 },
        );
        await expect(page.locator(terminalRowsSelector)).toContainText(
      ...
      await expect(page.locator(terminalRowsSelector)).not.toContainText(
        "CONNECTING",
        { timeout: 2000 },
      );
      await expect(page.locator(terminalRowsSelector)).not.toContainText(
  ...
  test.describe("Split Screen Feature", () => {
    test("should toggle split-screen terminal via button", async ({ page }) => {
      ...
      for (const button of buttons) {
        ...
        console.log(
          `Button: title="${title}", aria-label="${ariaLabel}", text="${text}"`,
        );
      }
    ...
    test("should maintain independent sessions in split terminals", async ({
      page,
    }) => {
      // Wait for any ongoing rate limit pause
      await waitForRateLimitLock();
