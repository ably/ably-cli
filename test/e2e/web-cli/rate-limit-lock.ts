  ...
  try {
    if (fs.existsSync(LOCK_FILE)) {
      ...
      if (!process.env.CI || process.env.VERBOSE_TESTS) {
        console.log(
          `[RateLimitLock] Read lock state: isLocked=${state.isLocked}, reason="${state.lockReason}", pid=${process.pid}`,
        );
      }
      return state;
    }
    ...
    if (!process.env.CI || process.env.VERBOSE_TESTS) {
      console.warn(
        `[RateLimitLock] Failed to read lock file at ${new Date().toISOString()}:`,
        error,
      );
    }
  }
...
function writeLockState(state: LockState): void {
  try {
    if (!process.env.CI || process.env.VERBOSE_TESTS) {
      console.log(
        `[RateLimitLock] Writing lock state: isLocked=${state.isLocked}, reason="${state.lockReason}", pid=${process.pid}`,
      );
    }
    fs.writeFileSync(LOCK_FILE, JSON.stringify(state, null, 2));
  } catch (error) {
    console.error(
      `[RateLimitLock] Failed to write lock file at ${new Date().toISOString()}:`,
      error,
    );
  }
}
...
export function acquireRateLimitLock(reason: string, duration: number): void {
  if (!process.env.CI || process.env.VERBOSE_TESTS) {
    console.log(
      `[RateLimitLock] Acquiring lock at ${new Date().toISOString()}, pid=${process.pid}`,
    );
    console.log(
      `[RateLimitLock] Lock reason: "${reason}", duration: ${duration}ms`,
    );
  ...
  if (!process.env.CI || process.env.VERBOSE_TESTS) {
    console.log(
      `[RateLimitLock] Lock acquired successfully at ${new Date().toISOString()}`,
    );
    console.log(
      `[RateLimitLock] Expected unlock at: ${new Date(state.expectedUnlockAt).toISOString()}`,
    );
  }
}
...
export function releaseRateLimitLock(): void {
  if (!process.env.CI || process.env.VERBOSE_TESTS) {
    console.log(
      `[RateLimitLock] Releasing lock at ${new Date().toISOString()}, pid=${process.pid}`,
    );
  ...
  if (!process.env.CI || process.env.VERBOSE_TESTS) {
    console.log(
      `[RateLimitLock] Lock released successfully at ${new Date().toISOString()}`,
    );
  }
}
...
export async function waitForRateLimitLock(): Promise<void> {
  ...
  if (!process.env.CI || process.env.VERBOSE_TESTS) {
    console.log(
      `[RateLimitLock] waitForRateLimitLock called at ${new Date().toISOString()}, pid=${process.pid}`,
    );
  ...
  while (checkCount < maxChecks) {
    ...
    if (!state.isLocked) {
      if (!process.env.CI || process.env.VERBOSE_TESTS) {
        console.log(
          `[RateLimitLock] Lock is available, proceeding at ${new Date().toISOString()}`,
        );
        console.log(
          `[RateLimitLock] Total wait time: ${Date.now() - startTime}ms`,
        );
      }
    ...
    if (state.lockedAt && Date.now() - state.lockedAt > 600000) {
      console.warn(
        `[RateLimitLock] Clearing stale lock at ${new Date().toISOString()} (older than 10 minutes)`,
      );
      console.warn(
        `[RateLimitLock] Lock was created at: ${new Date(state.lockedAt).toISOString()}`,
      );
    ...
    if (checkCount === 0 && (!process.env.CI || process.env.VERBOSE_TESTS)) {
      console.log(
        `[RateLimitLock] Waiting for rate limit lock to be released...`,
      );
      ...
      if (state.expectedUnlockAt) {
        ...
        console.log(
          `[RateLimitLock] Expected wait time: ${Math.round(remainingMs / 1000)}s`,
        );
      }
    }
    ...
    if (
      checkCount % 20 === 0 &&
      (!process.env.CI || process.env.VERBOSE_TESTS)
    ) {
      const currentState = readLockState();
      if (currentState.expectedUnlockAt) {
        const remainingMs = Math.max(
          0,
          currentState.expectedUnlockAt - Date.now(),
        );
        console.log(
          `[RateLimitLock] Still waiting... ${Math.round(remainingMs / 1000)}s remaining`,
        );
      }
    }
  }
  ...
  console.error(
    "[RateLimitLock] Timeout waiting for rate limit lock after 10 minutes",
  );
...
export function clearRateLimitLock(): void {
  if (!process.env.CI || process.env.VERBOSE_TESTS) {
    console.log(
      `[RateLimitLock] clearRateLimitLock called at ${new Date().toISOString()}, pid=${process.pid}`,
    );
  }
  ...
  try {
    if (fs.existsSync(LOCK_FILE)) {
      ...
      if (!process.env.CI || process.env.VERBOSE_TESTS) {
        console.log(
          `[RateLimitLock] Clearing lock file with state: isLocked=${state.isLocked}, reason="${state.lockReason}"`,
        );
      }
      fs.unlinkSync(LOCK_FILE);
      if (!process.env.CI || process.env.VERBOSE_TESTS) {
        console.log(
          `[RateLimitLock] Lock file cleared successfully at ${new Date().toISOString()}`,
        );
      }
    } else {
      if (!process.env.CI || process.env.VERBOSE_TESTS) {
    ...
    console.error(
      `[RateLimitLock] Failed to clear lock file at ${new Date().toISOString()}:`,
      error,
    );
  }
}
