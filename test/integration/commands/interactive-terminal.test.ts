...
describe("Interactive Mode - Terminal Integration Tests", () => {
  ...
  describe.skipIf(!process.stdout.isTTY || !!process.env.CI)(
    "Integration Tests",
    function () {
      it("should handle autocomplete with history navigation", function (done) {
        ...
        child.on("exit", (_code) => {
          ...
          expect(
            foundAutocomplete ||
              output.includes("accounts") ||
              output.includes("apps"),
          ).toBe(true);
          expect(foundHistory || output.includes("apps list")).toBe(true);
          done();
        });
      });
      ...
      it("should handle history search with autocomplete", function (done) {
        ...
        child.stdout.on("data", (data) => {
          ...
          if (
            data.toString().includes("(reverse-i-search)") ||
            data.toString().includes("reverse-i-search")
          ) {
