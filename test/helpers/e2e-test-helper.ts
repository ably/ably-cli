...
export function obfuscateSensitiveData(commandString: string): string {
  ...
  obfuscated = obfuscated.replace(
    ...
    "$1[REDACTED_API_KEY_SECRET]",
  );
  // eslint-disable-next-line unicorn/prefer-string-replace-all
  obfuscated = obfuscated.replace(
    /([a-zA-Z0-9\-_]+:)(?![a-zA-Z0-9\-_]*\.)[^\s'"]+/g,
    "$1[REDACTED_API_KEY_SECRET]",
  ); // keyId:keySecret (no app id part)
...
export function createAblyClient(): Ably.Rest {
  ...
  if (!E2E_API_KEY.includes(".") || !E2E_API_KEY.includes(":")) {
    console.warn(
      `[Client Lifecycle] Potential Issue: E2E_ABLY_API_KEY "${E2E_API_KEY.slice(0, 10)}..." appears structurally invalid (missing '.' or ':'). Proceeding anyway.`,
    );
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeoutHandle = setTimeout(() => {
      if (!resolved) {
        ...
        reject(
          new Error(
            `Process ${_obfuscatedCommand} timed out after ${timeoutMs}ms. STDOUT:\n${stdout}\nSTDERR:\n${stderr}`,
          ),
        );
      }
    }, timeoutMs);
...
export async function runLongRunningBackgroundProcess(
  ...
  throw new Error(
    `Failed to start process after ${retryCount + 1} attempts. Last error: ${lastError?.message}`,
  );
}
  ...
  const readinessPromise = new Promise<void>((resolveReady, rejectReady) => {
    ...
    signal.addEventListener("abort", () => {
      ...
      if (signal.reason) {
        ...
        rejectReady(
          new Error(`Process startup aborted: signal without reason`),
        );
      }
    });
      ...
      while (!signal.aborted) {
        ...
        try {
          ...
          if (output.length === lastOutputLength) {
            ...
            if (
              consecutiveNoChangeCount >= maxConsecutiveNoChange &&
              output.length === 0
            ) {
          ...
          if (readySignal && output.length > 0) {
            ...
            const signalFound =
              ...
              (readySignal.includes("ready") &&
                normalizedOutput.includes("ready")) ||
              (readySignal.includes("listening") &&
                normalizedOutput.includes("listening")) ||
              (readySignal.includes("subscribing") &&
                normalizedOutput.includes("subscribing")) ||
              (readySignal.includes("entered") &&
                (normalizedOutput.includes("entered") ||
                  normalizedOutput.includes("✓"))) ||
              // Special handling for presence/member signals
              (readySignal.toLowerCase().includes("member") &&
                normalizedOutput.includes("member")) ||
              (readySignal.toLowerCase().includes("presence") &&
                normalizedOutput.includes("presence"));
          ...
          if (hasCriticalError || (hasGenericError && pollCount < 10)) {
            ...
            rejectReady(
              new Error(
                `Process ${command} failed with error pattern in output. Full Output:\n${errorOutput}`,
              ),
            );
            return;
          }
          ...
          if (pollCount > 20) {
            // Give it some time before logging
    ...
    try {
      ...
      outputStream.on("error", (err) => {
        ...
        if (!controller.signal.aborted) {
          controller.abort(
            `OutputStream error for ${outputPath}: ${err.message}`,
          );
        }
      });
 ...
 */
export function trackTestCommand(
  command: string,
  outputPath?: string,
  result?: { exitCode: number | null; stdout?: string; stderr?: string },
): void {
 ...
 */
export async function displayTestFailureDebugOutput(
