...
export class CliRunner extends EventTarget {
  ...
  isRunning(): boolean {
    return (
      this.process !== null && this.processExitCode === null && !this.killed
    );
  }
  ...
  private async spawnProcess(): Promise<void> {
    ...
    const isCli =
      this.cmd.startsWith("ably ") || this.cmd.startsWith("bin/run.js");
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      const checkReady = () => {
        if (this.killed || this.processExitCode !== null) {
          reject(
            new Error(
              `Process exited before ready signal detected. Exit code: ${this.processExitCode}`,
            ),
          );
          return;
        }
        ...
        if (Date.now() - startTime > timeoutMs) {
          reject(
            new Error(
              `Timeout waiting for ready signal "${matcher}". Output:\n${this.combinedBuffer.slice(-1000)}`,
            ),
          );
          return;
        }
