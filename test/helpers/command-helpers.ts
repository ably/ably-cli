...
export function getOutputFile(suffix = ""): string {
  ...
  const testName =
    (currentTest as TestContext | undefined)?.fullTitle() || "unknown-test";
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    const checkOutput = () => {
      if (!runner.isRunning()) {
        reject(
          new Error(
            `Process exited before pattern "${matcher}" was found. Exit code: ${runner.exitCode()}`,
          ),
        );
        return;
      }
      ...
      if (Date.now() - startTime > timeoutMs) {
        reject(
          new Error(
            `Timeout waiting for pattern "${matcher}". Recent output:\n${runner.combined().slice(-500)}`,
          ),
        );
        return;
      }
  ...
  return new Promise<unknown[]>((resolve, reject) => {
    ...
    const checkEvents = () => {
      if (!runner.isRunning()) {
        reject(
          new Error(
            `Process exited before ${expectedCount} events were received. Exit code: ${runner.exitCode()}`,
          ),
        );
        return;
      }
      ...
      if (Date.now() - startTime > timeoutMs) {
        ...
        reject(
          new Error(
            `Timeout waiting for ${expectedCount} JSON events. Found ${filtered.length}. Recent output:\n${runner.combined().slice(-1000)}`,
          ),
        );
        return;
      }
