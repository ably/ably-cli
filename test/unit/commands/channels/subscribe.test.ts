...
describe("channels:subscribe command", () => {
  ...
  describe("token streaming mode", () => {
    ...
    it("should fall through to normal display for message.update during streaming mode", async () => {
      const commandPromise = runCommand(
        ["channels:subscribe", "test-channel", "--token-streaming"],
        import.meta.url,
      );
      ...
      // First a create, then an update (which should use normal display)
      mockSubscribeCallback!({
        action: "message.create",
      ...
      // The update should fall through to normal display with Name: label
      expect(stdout).toContain("Initial");
      expect(stdout).toContain("Updated content");
      expect(stdout).toContain("Name:");
    });
