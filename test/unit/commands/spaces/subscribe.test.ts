...
describe("spaces:subscribe command", () => {
  ...
  describe("functionality", () => {
    it("should subscribe to space updates and output events", async () => {
    ...
    it("should output JSON event with members array", async () => {
      ...
      expect(result.space.members).toBeDefined();
      expect(result.space.members).toBeInstanceOf(Array);
      expect(result.space.members[0].clientId).toBe("user-1");
    });
  });
