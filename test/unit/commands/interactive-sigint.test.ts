...
describe("Interactive Mode - SIGINT Handling", () => {
  ...
  it.skip(
    "should handle Ctrl+C on empty prompt - FLAKY in non-TTY environments",
    (done) => {
      ...
      child.stdout.on("data", (data) => {
        ...
        if (
          sigintSent &&
          !exitSent &&
          (data.toString().includes("^C") ||
            data.toString().includes("Signal received"))
        ) {
          exitSent = true;
  ...
  // eslint-disable-next-line vitest/no-disabled-tests
  it.skip(
    "should handle Ctrl+C with partial command input - FLAKY in non-TTY environments",
    (done) => {
      // SKIPPED: This test is flaky in non-TTY environments
      // The interactive mode readline SIGINT handler may not work properly
      // See: https://github.com/ably/cli/issues/70
