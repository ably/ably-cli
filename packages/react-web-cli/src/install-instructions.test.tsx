...
describe("Install Instructions", () => {
  ...
  test("connection messages have appropriate titles", () => {
    expect(getConnectionMessage("connectionFailed").title).toBe(
      "CONNECTION FAILED",
    );
    expect(getConnectionMessage("serverDisconnect").title).toBe(
      "SERVER DISCONNECTED",
    );
    expect(getConnectionMessage("maxReconnects").title).toBe(
      "SERVICE UNAVAILABLE",
    );
    expect(getConnectionMessage("capacityReached").title).toBe("AT CAPACITY");
    expect(getConnectionMessage("connectionTimeout").title).toBe(
      "CONNECTION TIMEOUT",
    );
    expect(getConnectionMessage("reconnectCancelled").title).toBe(
      "RECONNECTION CANCELLED",
    );
    expect(getConnectionMessage("reconnectingWithInstall").title).toBe(
      "RECONNECTING",
    );
  });
