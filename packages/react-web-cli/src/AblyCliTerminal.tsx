...
const AblyCliTerminalInner = (
    ...
    (message: any) => {
      ...
      if (message.type === "status") {
        ...
        if (message.payload === "error" || message.payload === "disconnected") {
          ...
          updateConnectionStatusAndExpose(message.payload);
          ...
          if (term.current && message.payload === "disconnected") {
            ...
            statusBoxReference.current = drawBox(
              term.current,
              boxColour.red,
              title,
              lines,
      ...
      console.warn("[WebSocket] Unrecognized control message:", message);
    },
    [
      sessionId,
      onSessionId,
  ...
  const clearSecondaryStatusDisplay = useCallback(() => {
    if (secondarySpinnerIntervalReference.current) {
      clearInterval(secondarySpinnerIntervalReference.current);
      secondarySpinnerIntervalReference.current = null;
    }
    secondarySpinnerIndexReference.current = 0;
    if (secondaryStatusBoxReference.current && secondaryTerm.current) {
      clearBox(secondaryStatusBoxReference.current);
      secondaryStatusBoxReference.current = null;
    }
    setSecondaryOverlay(null);
  ...
  const handleSecondaryPtyData = useCallback(
    (data: string) => {
      if (!isSecondarySessionActive) {
        ...
        if (hasShellPrompt || hasAblyPrompt) {
          debugLog(
            "[AblyCLITerminal] [Secondary] Prompt detected – session active",
          );
  ...
  const startConnectingAnimation = useCallback(
    (isRetry: boolean) => {
      ...
      if (term.current && !isRetry && !sessionIdReference.current) {
      ...
      if (term.current) {
        ...
        spinnerPrefixReference.current = statusText; // Store base text for spinner line

        spinnerIndexReference.current = 0;
        const initialSpinnerChar =
          spinnerFrames[spinnerIndexReference.current % spinnerFrames.length];
        if (statusBoxReference.current) {
          // Initial spinner render
          const fullLineText = `${initialSpinnerChar} ${spinnerPrefixReference.current}`;
          updateLine(statusBoxReference.current, 0, fullLineText, titleColor);
        }

        spinnerIntervalReference.current = setInterval(() => {
          // Stop spinner when no longer in connecting states
          const currentState = connectionStatusReference.current;
          ...
          spinnerIndexReference.current += 1;
          const frame =
            spinnerFrames[spinnerIndexReference.current % spinnerFrames.length];
          const lineContent = `${frame} ${spinnerPrefixReference.current}`;

          // Update ANSI box if still present
          if (statusBoxReference.current) {
            updateLine(statusBoxReference.current, 0, lineContent, titleColor);
          }

          // Update overlay - preserve drawer or add it if showInstallInstructions is true
          setOverlay((previous) => {
            if (!previous) return previous;
            const newLines = [...previous.lines];
      ...
      const initialSpinnerChar =
        spinnerFrames[spinnerIndexReference.current % spinnerFrames.length];
      const initialLines = [
        `${initialSpinnerChar} ${spinnerPrefixReference.current}`,
        "",
      ]; // Include empty line for countdown
  ...
  const connectWebSocket = useCallback(() => {
    ...
    if (
      !showManualReconnectPromptReference.current &&
      socketReference.current &&
      ...
      debugLog(
        "⚠️ DIAGNOSTIC: Socket already open/connecting, skipping connection attempt",
      );
      return;
    } else if (
      socketReference.current &&
      ...
      debugLog(
        "⚠️ DIAGNOSTIC: Existing socket state",
        socketReference.current.readyState,
        "→ will proceed to open new socket",
      );
    }
    ...
    if (
      ...
      debugLog(
        "⚠️ DIAGNOSTIC: Closing existing socket before creating new one",
      );
    ...
    if (!connectionStartTime) {
      ...
      clearInstallInstructionsTimer();
      installInstructionsTimerReference.current = setTimeout(() => {
        setShowInstallInstructions(true);
      }, SHOW_INSTALL_AFTER_MS);
    }

    updateConnectionStatusAndExpose(
      grIsCancelledState() || grIsMaxAttemptsReached()
        ? "disconnected"
        : grGetAttempts() > 0
          ? "reconnecting"
          : "connecting",
    ...
    // reconnect.
    reconnectScheduledThisCycleReference.current = false;

    // Set up connection timeout
    clearConnectionTimeout(); // Clear any existing timeout
    connectionTimeoutReference.current = setTimeout(() => {
      debugLog(
        "⚠️ CONNECTION TIMEOUT: WebSocket connection attempt timed out after",
      ...
      if (
        socketReference.current &&
        socketReference.current.readyState === WebSocket.CONNECTING
      ) {
        debugLog(
          "⚠️ CONNECTION TIMEOUT: Closing socket that is still in CONNECTING state",
        );
        socketReference.current.close(4003, "connection-timeout");
        ...
        socketReference.current.dispatchEvent(timeoutError);
      }
    }, CONNECTION_TIMEOUT_MS);

    // new WebSocket created
    debugLog(
      "⚠️ DIAGNOSTIC: WebSocket connection initiation complete. sessionId:",
      sessionId,
      "showManualReconnectPrompt:",
      showManualReconnectPromptReference.current,
    );
    ...
    isVisible,
    sessionId,
    showManualReconnectPromptReference,
    ...
    async (event: MessageEvent) => {
      ...
      try {
        ...
        if (filteredData.length > 0) {
          debugLog(
            `⚠️ DIAGNOSTIC: Filtered data received, length: ${filteredData.length}`,
          );
          ...
          if (filteredData.includes("ABLY_CTRL:")) {
            ...
            while (currentPos < filteredData.length) {
              ...
              if (ctrlIndex === -1) {
                ...
                regularOutput += filteredData.slice(Math.max(0, currentPos));
                break;
              }
              ...
              let messageEnd = filteredData.indexOf("\n", ctrlIndex);
              if (messageEnd === -1)
                messageEnd = filteredData.indexOf("\r", ctrlIndex);
              ...
              if (
                currentPos < filteredData.length &&
                (filteredData[currentPos] === "\n" ||
                  filteredData[currentPos] === "\r")
              ) {
                if (
                  filteredData[currentPos] === "\r" &&
                  currentPos + 1 < filteredData.length &&
                  filteredData[currentPos + 1] === "\n"
                ) {
                  currentPos += 2; // Skip \r\n
                } else {
  ...
  const handleWebSocketError = useCallback(
    (event: Event) => {
      ...
      if (event instanceof ErrorEvent) {
        console.error(
          `[AblyCLITerminal] WebSocket ErrorEvent: message=${event.message}, filename=${event.filename}, lineno=${event.lineno}, colno=${event.colno}`,
        );
      }
      ...
      if (grIsCancelledState() || grIsMaxAttemptsReached()) {
        ...
        reconnectScheduledThisCycleReference.current = true; // Prevent double handling
        return;
      }

      if (!reconnectScheduledThisCycleReference.current) {
        ...
        debugLog(
          "[AblyCLITerminal handleWebSocketError] Triggering auto-reconnect sequence. Current grAttempts (before increment):",
          grGetAttempts(),
        );
        ...
        reconnectScheduledThisCycleReference.current = true;
      }
    },
    [
      updateConnectionStatusAndExpose,
    ...
    (event: CloseEvent) => {
      ...
      if (NON_RECOVERABLE_CLOSE_CODES.has(event.code)) {
        ...
        if (term.current) {
          ...
          if (
            event.code === 4009 ||
            event.reason?.toLowerCase().includes("capacity")
          ) {
            message = getConnectionMessage("capacityReached");
          } else if (
            event.code === 4003 ||
            event.reason?.toLowerCase().includes("rate limit")
          ) {
            message = {
        ...
        debugLog(
          "[AblyCLITerminal] Purging sessionId due to non-recoverable close. code:",
          event.code,
          "sessionId:",
          sessionId,
        );
      ...
      if (grIsCancelledState() || grIsMaxAttemptsReached()) {
        ...
        setShowManualReconnectPrompt(true);
        return;
      } else if (!reconnectScheduledThisCycleReference.current) {
        debugLog(
          "[AblyCLITerminal handleWebSocketClose] Scheduling reconnect. Current grAttempts (before increment):",
          grGetAttempts(),
        );
        updateConnectionStatusAndExpose("reconnecting");
        startConnectingAnimation(true);
        ...
        grIncrement();
        debugLog(
          "[AblyCLITerminal handleWebSocketClose] grIncrement called. Current grAttempts (after increment):",
          grGetAttempts(),
        );
  ...
  useEffect(() => {
    // Setup terminal
    if (!term.current && rootReference.current) {
      ...
      if (typeof term.current.attachCustomKeyEventHandler === "function") {
        term.current.attachCustomKeyEventHandler(
          (event: KeyboardEvent): boolean => {
            ...
            debugLog(
              `⚠️ DIAGNOSTIC: KeyboardEvent - key: "${event.key}", code: "${event.code}", ctrl: ${event.ctrlKey}, shift: ${event.shiftKey}`,
            );
            ...
            if (event.key === "Tab" && !event.ctrlKey && !event.altKey) {
              ...
              debugLog(
                `⚠️ DIAGNOSTIC: TAB intercepted, sending special message for autocomplete`,
              );
            ...
            if (event.key === "ArrowUp" && !event.ctrlKey && !event.altKey) {
              event.preventDefault();
              debugLog(
                `⚠️ DIAGNOSTIC: UP arrow intercepted, sending special message for history`,
              );
            ...
            if (event.key === "ArrowDown" && !event.ctrlKey && !event.altKey) {
              event.preventDefault();
              debugLog(
                `⚠️ DIAGNOSTIC: DOWN arrow intercepted, sending special message for history`,
              );
            ...
            if (
              ...
              event.preventDefault();
              debugLog(
                `⚠️ DIAGNOSTIC: Ctrl+R intercepted, sending special message for history search`,
              );
      ...
      term.current.onData((data: string) => {
        ...
        switch (data) {
          ...
          default: {
            if (data.startsWith("\u001B")) keyName = "ESC_SEQUENCE";
          }
        }

        debugLog(
          `⚠️ DIAGNOSTIC: Terminal onData - Key: ${keyName}, Bytes: "${bytes}", Length: ${data.length}`,
        );

        // Special handling for Enter key
        if (data === "\r") {
          const latestStatus = connectionStatusReference.current;
          debugLog(
            `⚠️ DIAGNOSTIC: Enter key pressed, status: ${latestStatus}, reconnectPrompt: ${showManualReconnectPromptReference.current}`,
          );

          // Manual prompt visible: attempt manual reconnect even if an old socket is open
          if (showManualReconnectPromptReference.current) {
            // manual reconnect
            ...
            if (
              ...
              try {
                socketReference.current.close(1000, "manual-reconnect");
              } catch {
                /* ignore */
              }
              socketReference.current = null; // Make sure the reference is cleared
            }

            // Give the browser a micro-task to mark socket CLOSED before reconnect
            setTimeout(() => {
              debugLog(
                "[AblyCLITerminal] [setTimeout] Starting fresh reconnect sequence",
              );
              grResetState();
              ...
              debugLog(
                "[AblyCLITerminal] [setTimeout] Invoking latest connectWebSocket",
              );
              connectWebSocketReference.current?.();
              debugLog(
                "[AblyCLITerminal] [setTimeout] connectWebSocket invoked",
              );
            ...
            debugLog(
              "[AblyCLITerminal] Enter pressed for manual reconnect. sessionId:",
              sessionId,
            );
            return;
          }
          ...
          if (latestStatus === "reconnecting" && !grIsCancelledState()) {
            ...
            debugLog(
              "[AblyCLITerminal] Enter pressed during auto-reconnect: Cancelling.",
            );
            grCancelReconnect();
            grResetState();
            clearAnimationMessages();
        ...
        if (data === "\r") {
          ...
          if (cursorPosition > 0) {
            currentInputLine =
              currentInputLine.slice(0, cursorPosition - 1) +
              currentInputLine.slice(cursorPosition);
            cursorPosition--;
          }
        } else if (
          data.length === 1 &&
          ...
          currentInputLine =
            currentInputLine.slice(0, cursorPosition) +
            data +
            currentInputLine.slice(cursorPosition);
          cursorPosition++;
        }
        ...
        if (
          ...
          debugLog(
            `⚠️ DIAGNOSTIC: Sending to WebSocket - Key: ${keyName}, Bytes: "${bytes}"`,
          );
          socketReference.current.send(data);
        } else if (data === "\r") {
          debugLog(
            `⚠️ DIAGNOSTIC: Socket not open, not sending carriage return`,
          );
          ...
          debugLog(
            `⚠️ DIAGNOSTIC: Socket not open, and not a carriage return, ignoring input`,
          );
        }
      });

      debugLog("⚠️ DIAGNOSTIC: Opening terminal in DOM element");
      term.current.open(rootReference.current);
      try {
        ...
        setTimeout(() => {
          try {
            ...
            console.warn("Error fitting addon initial timeout:", error);
          }
        }, 100);
      } catch (error) {
      ...
      grSetCountdownCallback((remainingMs) => {
        ...
        if (connectionStatusReference.current !== "reconnecting") return;

        const seconds = Math.ceil(remainingMs / 1000);
        const messagePlain = `Next attempt in ${seconds}s...`;
        const messageWithCancel = `${messagePlain}  (Press ⏎ to cancel)`;
        setCountdownMessage(messagePlain);

        if (
          term.current &&
          statusBoxReference.current &&
          statusBoxReference.current.content.length > 1
        ) {
        ...
        setOverlay((previous) => {
          ...
          if (newLines.length === 1) {
            newLines.push(messageWithCancel);
          } else {
            newLines[1] = messageWithCancel;
          }
    ...
    if (maxReconnectAttempts && maxReconnectAttempts !== grGetMaxAttempts()) {
      debugLog(
        "[AblyCLITerminal] Setting max reconnect attempts to",
        maxReconnectAttempts,
      );
  ...
  useEffect(() => {
    if (!resumeOnReload || globalThis.window === undefined) return;
    ...
    if (sessionId) {
      ...
      globalThis.sessionStorage.removeItem(`ably.cli.sessionId.${urlDomain}`);
      globalThis.sessionStorage.removeItem(
        `ably.cli.credentialHash.${urlDomain}`,
      );
    }
  ...
  useEffect(() => {
    if (overlay && rootReference.current) {
      // Wait till next tick to ensure DOM rendered
      requestAnimationFrame(() => {
        try {
          const _rootRect = rootReference.current?.getBoundingClientRect();
          const _parentRect =
            rootReference.current?.parentElement?.getBoundingClientRect();
          const overlayElement = rootReference.current?.querySelector(
            ".ably-overlay",
          ) as HTMLElement | null;
          const _overlayRect = overlayElement?.getBoundingClientRect();

          // layout diagnostics removed
        } catch (error) {
          // Swallow errors silently in production but log in dev
          console.error("Overlay diagnostics error", error);
        }
      });
    }
  }, [overlay]);
  ...
  const clearInactivityTimer = useCallback(() => {
    if (inactivityTimerReference.current) {
      clearTimeout(inactivityTimerReference.current);
      inactivityTimerReference.current = null;
    }
  }, []);
  ...
  useEffect(() => {
    const initializeSession = async () => {
      ...
      const storedHash = globalThis.sessionStorage.getItem(
        `ably.cli.credentialHash.${urlDomain}`,
      );
  ...
  const connectWebSocketReference = useRef(connectWebSocket);
  useEffect(() => {
    connectWebSocketReference.current = connectWebSocket;
  }, [connectWebSocket]);
  ...
  const handleDrag = useCallback(
    (e: MouseEvent) => {
      if (!isDragging || !outerContainerReference.current) return;

      const containerRect =
        outerContainerReference.current.getBoundingClientRect();
  ...
  useEffect(() => {
    const handleGlobalResize = debounce(() => {
      try {
        ...
        console.warn("Error in global resize handler:", error);
      }
    }, 200);
    ...
    if (resumeOnReload && globalThis.window !== undefined) {
      if (isSplit) {
        ...
        if (!secondarySessionId) {
          globalThis.sessionStorage.removeItem("ably.cli.secondarySessionId");
          debugLog(
            "[AblyCLITerminal] Removed secondarySessionId from sessionStorage",
          );
        }
      }
    }
  ...
  const connectSecondaryWebSocketReference = useRef<(() => void) | null>(null);
  ...
  const connectSecondaryWebSocket = useCallback(() => {
    ...
    if (!secondaryTerm.current || !secondaryRootReference.current) {
      return;
    }
    ...
    if (
      secondaryTerm.current && // Only clear terminal if this is a new session (not resuming)
      !secondarySessionId
    ) {
    ...
    newSocket.addEventListener("error", (event) => {
      ...
      console.error(
        "[AblyCLITerminal] [Secondary] WebSocket URL was:",
        websocketUrl,
      );
      console.error(
        "[AblyCLITerminal] [Secondary] WebSocket state:",
    ...
    newSocket.addEventListener("close", (event) => {
      ...
      if (NON_RECOVERABLE_CLOSE_CODES.has(event.code)) {
        ...
        if (resumeOnReload && globalThis.window !== undefined) {
          globalThis.sessionStorage.removeItem("ably.cli.secondarySessionId");
          debugLog(
            "[AblyCLITerminal] [Secondary] Cleared invalid session ID due to non-recoverable error",
          );
        }
      }
  ...
  useEffect(() => {
      ...
      setTimeout(() => {
        if (fitAddon.current) {
          try {
            ...
            console.warn("Error fitting primary terminal after split:", error);
          }
        }
        if (secondaryFitAddon.current) {
          try {
            ...
            console.warn(
              "Error fitting secondary terminal after split:",
              error,
            );
          }
        }
    ...
    if (!isSplit || !secondaryRootReference.current || secondaryTerm.current) {
    ...
    debugLog(
      "[AblyCLITerminal] Setting up custom key handler for secondary terminal",
    );
    secondaryTerm.current.attachCustomKeyEventHandler(
      (event: KeyboardEvent): boolean => {
        // Only handle special keys when connected and session is active
        if (
        ...
        debugLog(
          `[AblyCLITerminal] [Secondary] KeyboardEvent - key: "${event.key}", code: "${event.code}", ctrl: ${event.ctrlKey}`,
        );
        ...
        if (event.key === "Tab" && !event.ctrlKey && !event.altKey) {
          event.preventDefault();
          debugLog(
            `[AblyCLITerminal] [Secondary] TAB intercepted, sending special message for autocomplete`,
          );

          secondarySocketReference.current.send(
            JSON.stringify({
              type: "readline-control",
              action: "complete",
        ...
        if (event.key === "ArrowUp" && !event.ctrlKey && !event.altKey) {
          event.preventDefault();
          debugLog(
            `[AblyCLITerminal] [Secondary] UP arrow intercepted, sending special message for history`,
          );
        ...
        if (event.key === "ArrowDown" && !event.ctrlKey && !event.altKey) {
          event.preventDefault();
          debugLog(
            `[AblyCLITerminal] [Secondary] DOWN arrow intercepted, sending special message for history`,
          );
        ...
        if (
          ...
          event.preventDefault();
          debugLog(
            `[AblyCLITerminal] [Secondary] Ctrl+R intercepted, sending special message for history search`,
          );
    ...
    secondaryTerm.current.onData((data: string) => {
      ...
      if (data === "\r") {
        ...
        if (secondaryCursorPosition > 0) {
          secondaryCurrentInputLine =
            secondaryCurrentInputLine.slice(0, secondaryCursorPosition - 1) +
            secondaryCurrentInputLine.slice(secondaryCursorPosition);
        ...
        // Regular printable character
        secondaryCurrentInputLine =
          secondaryCurrentInputLine.slice(0, secondaryCursorPosition) +
          data +
          secondaryCurrentInputLine.slice(secondaryCursorPosition);
        secondaryCursorPosition++;
      }
      ...
      if (
        ...
        secondarySocketReference.current.send(data);
      }
    });

    // Open terminal in the DOM
    secondaryTerm.current.open(secondaryRootReference.current);
      ...
      try {
        ...
        setTimeout(() => {
          try {
            ...
            console.warn("Error fitting secondary addon delayed fit:", error);
          }
        }, 200);
      } catch (error) {
        console.error("Error during initial secondary terminal fit:", error);
      }
    ...
    const handleResize = () => {
      if (secondaryFitAddon.current) {
        try {
          ...
          console.warn("Error fitting secondary terminal on resize:", error);
        }
      }
    };

    window.addEventListener("resize", handleResize);
    secondaryTermCleanupReference.current = () => {
    ...
    return () => {
      ...
      if (secondaryTermCleanupReference.current) {
        secondaryTermCleanupReference.current();
      }
  ...
  const updateSecondaryConnectionStatus = useCallback(
    (status: ConnectionStatus) => {
