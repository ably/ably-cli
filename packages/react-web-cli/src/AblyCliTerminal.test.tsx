...
describe("AblyCliTerminal - Connection Status and Animation", () => {
  ...
  beforeEach(() => {
    ...
    mockDrawBox.mockReturnValue({
      ...
      content: [],
      write: vi.fn(),
      height: 0, // Minimal TerminalBox structure
    });
  ...
  test("displays error box for non-recoverable server close (e.g., 4001) and persists it", async () => {
    ...
    expect(onConnectionStatusChangeMock).toHaveBeenLastCalledWith(
      "disconnected",
    );
  });
  ...
  test('handles "error" status from server', async () => {
    ...
    await act(async () => {
      ...
      mockSocketInstance.triggerEvent("message", {
        data: createControlMessage(errorMessage),
      });
    ...
    expect(mockWriteln).toHaveBeenCalledWith(
      expect.stringContaining("--- Error: Test error reason ---"),
    );
  });
  ...
  test("clears animation on WebSocket close", async () => {
    ...
    await act(async () => {
      mockSocketInstance.triggerEvent("message", {
        data: createControlMessage({ type: "status", payload: "connecting" }),
      });
  ...
  test("uses exponential backoff strategy for reconnection attempts", async () => {
    ...
    await act(async () => {
      if (!mockSocketInstance)
        throw new Error("No mock socket for initial close");
    ...
    expect(vi.mocked(GlobalReconnect.scheduleReconnect)).toHaveBeenCalledTimes(
      1,
    );
  ...
  test("can cancel automatic reconnection via Enter key", async () => {
    ...
    const onDataHandler = vi.mocked(mockOnData).mock.calls[0][0] as (
      data: string,
    ) => void;
    act(() => {
      onDataHandler("\r");
  ...
  test.skip("shows installation tip during reconnection after 6 seconds", async () => {
    ...
    try {
      ...
      await waitFor(() => {
        expect(screen.getByTestId("ably-overlay")).toBeInTheDocument();
        const overlayText =
          screen.getByTestId("ably-overlay").textContent || "";
  ...
  test.skip("manual reconnect resets attempt counter after max attempts reached - skipped due to CI timing issues", async () => {
    ...
    await waitFor(() => {
      const maxReconnectsCall = mockDrawBox.mock.calls.find(
        (call) => call[2] === "SERVICE UNAVAILABLE",
      );
    ...
    expect(
      vi.mocked(GlobalReconnect.successfulConnectionReset),
    ).toHaveBeenCalled();
    ...
    await waitFor(() => {
      const connectingCall = mockDrawBox.mock.calls.find(
  ...
  test("invokes onSessionId when hello message received", async () => {
    ...
    act(() => {
      if (!mockSocketInstance)
        throw new Error("mockSocketInstance not initialized");
      mockSocketInstance.triggerEvent("message", {
        data: createControlMessage({ type: "hello", sessionId: "session-123" }),
      });
    });
  ...
  test("includes stored sessionId in auth payload when resumeOnReload enabled", async () => {
    ...
    const payloadWithSessionId = authPayloads.find(
      (p) => p.sessionId === "resume-123",
    );
  ...
  test("increments only once when both error and close events fire for the same failure", async () => {
    ...
    act(() => {
      if (!mockSocketInstance)
        throw new Error("mockSocketInstance not initialised");
      mockSocketInstance.triggerEvent("error", new Event("error"));
      mockSocketInstance.triggerEvent("close", {
        code: 1006,
  ...
  test.skip("connection timeout triggers error after 30 seconds", async () => {
    ...
    expect(mockSocketInstance.close).toHaveBeenCalledWith(
      4003,
      "connection-timeout",
    );
  ...
  test("suppresses fragmented hijack meta JSON chunks", async () => {
    ...
    await act(async () => {
      ...
      mockSocketInstance.triggerEvent("message", {
        data: '{"stream":true,"std',
      });
      await new Promise((resolve) => setTimeout(resolve, 10));

      // Second fragment (closing half) – should also be ignored
      mockSocketInstance.triggerEvent("message", {
        data: 'in":true,"stdout":true,"stderr":true,"hijack":true}',
      });
      await new Promise((resolve) => setTimeout(resolve, 10));
    });
  ...
  test("split icon is visible and toggles split-screen UI", async () => {
    ...
    expect(
      await screen.findByTestId("terminal-container-secondary"),
    ).toBeInTheDocument();

    // Split button should be gone in split mode
    expect(
      screen.queryByRole("button", { name: /Split terminal/i }),
    ).toBeNull();
    ...
    expect(
      await screen.findByRole("button", { name: /Split terminal/i }),
    ).toBeInTheDocument();
  });
  ...
  test("split-screen initializes a secondary terminal with its own WebSocket session", async () => {
    ...
    await waitFor(() =>
      expect(webSocketInstancesMock).toHaveBeenCalledTimes(1),
    );
  ...
  test("closing primary terminal in split mode keeps secondary terminal active and moves it to primary position", async () => {
    ...
    expect(await screen.findByTestId("terminal-container")).toBeInTheDocument();
    expect(
      screen.queryByTestId("terminal-container-secondary"),
  ...
  test("split mode state is preserved across reloads when resumeOnReload is true", async () => {
    ...
    getItemMock.mockImplementation((key: string) => {
      ...
      if (key === "ably.cli.secondarySessionId")
        return "mock-secondary-session-id";
      return null;
    });
  ...
  test.skip("onConnectionStatusChange only reports status for the primary terminal in split-screen mode", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByTestId("terminal-container-secondary"),
      ).toBeInTheDocument();
    });
...
describe("AblyCliTerminal - Credential Validation", () => {
  ...
  test("stores credential hash when new session is created", async () => {
    const customConfig = createTestSignedConfig("test-key-123", "test-token-456");
    renderTerminal({
...
describe("AblyCliTerminal - Cross-Domain Security", () => {
  ...
  test("credentials are not shared between different serverUrls", async () => {
    ...
    const secureConfig = createTestSignedConfig("secure-key-123", "secure-token-456");
  ...
  test("credentials are properly scoped per domain", async () => {
    ...
    const authPayloads = mockSend.mock.calls.map((call) => JSON.parse(call[0]));
    const payloadWithSessionId = authPayloads.find(
      (p) => p.sessionId === "ably-session-123",
    );
    expect(payloadWithSessionId).toBeTruthy();
  ...
  test("localStorage auth settings are not shared between domains", async () => {
    ...
    const differentConfig = createTestSignedConfig("different-key", "different-token");
    renderTerminal({
