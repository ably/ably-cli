...
Authentication in tests uses different mechanisms depending on the layer. See [ENV_VARIABLES_USAGE.md](ENV_VARIABLES_USAGE.md) for full details on `ABLY_API_KEY`, `ABLY_TOKEN`, `ABLY_ACCESS_TOKEN`, and other auth env vars.
...
- [ENV_VARIABLES_USAGE.md](ENV_VARIABLES_USAGE.md) — Complete reference of all environment variables including auth (`ABLY_API_KEY`, `ABLY_TOKEN`, `ABLY_ACCESS_TOKEN`), duration (`ABLY_CLI_DEFAULT_DURATION`), and test mode (`ABLY_CLI_TEST_MODE`)
