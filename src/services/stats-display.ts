...
export class StatsDisplay {
  private cumulativeStats: {
    ...
    messages: {
      data: { delivered: number; published: number };
      delivered: number;
      published: number;
    };
    tokenRequests: {
  ...
  public display(stats: StatsDisplayData): void {
    ...
    if (this.options.live) {
      ...
      if (
        this.lastStats &&
        JSON.stringify(this.lastStats) === JSON.stringify(stats)
      ) {
        return;
      }
  ...
  private calculatePeakRates(stats: StatsDisplayData): void {
    ...
    this.peakRates.messages.published = Math.max(
      this.peakRates.messages.published,
      publishedMessages / timeDiff,
    );
    this.peakRates.messages.delivered = Math.max(
      this.peakRates.messages.delivered,
      deliveredMessages / timeDiff,
    );

    // Calculate rates for connections
    const connections = getEntry("connections.all.peak");
    this.peakRates.connections = Math.max(
      this.peakRates.connections,
      connections / timeDiff,
    );

    // Calculate rates for channels
    const channels = getEntry("channels.peak");
    this.peakRates.channels = Math.max(
      this.peakRates.channels,
      channels / timeDiff,
    );
    ...
    this.peakRates.apiRequests = Math.max(
      this.peakRates.apiRequests,
      apiRequests / timeDiff,
    );
    ...
    this.peakRates.tokenRequests = Math.max(
      this.peakRates.tokenRequests,
      tokenRequests / timeDiff,
    );
  }
  ...
  private displayCumulativeStats(): void {
    ...
    const apiSuccessRate =
      this.cumulativeStats.apiRequests.total > 0
        ? (
    ...
    const tokenSuccessRate =
      this.cumulativeStats.tokenRequests.total > 0
        ? (
  ...
  private updateCumulativeStats(stats: StatsDisplayData): void {
    ...
    this.cumulativeStats.messages.published += getEntry(
      "messages.inbound.all.messages.count",
    );
    this.cumulativeStats.messages.delivered += getEntry(
      "messages.outbound.all.messages.count",
    );
    this.cumulativeStats.messages.data.published += getEntry(
      "messages.inbound.all.messages.data",
    );
    this.cumulativeStats.messages.data.delivered += getEntry(
      "messages.outbound.all.messages.data",
    );

    this.cumulativeStats.connections.peak = Math.max(
      this.cumulativeStats.connections.peak,
      getEntry("connections.all.peak"),
    );
    this.cumulativeStats.connections.current = Math.round(
      getEntry("connections.all.mean"),
    );
    this.cumulativeStats.connections.opened += getEntry(
      "connections.all.opened",
    );
    this.cumulativeStats.connections.total += getEntry("connections.all.count");

    this.cumulativeStats.channels.peak = Math.max(
      this.cumulativeStats.channels.peak,
      getEntry("channels.peak"),
    );
    this.cumulativeStats.channels.current = Math.round(
      getEntry("channels.mean"),
    );
    this.cumulativeStats.channels.opened += getEntry("channels.opened");
    this.cumulativeStats.channels.total += getEntry("channels.count");

    this.cumulativeStats.apiRequests.succeeded += getEntry(
      "apiRequests.all.succeeded",
    );
    this.cumulativeStats.apiRequests.failed += getEntry(
      "apiRequests.all.failed",
    );
    this.cumulativeStats.apiRequests.refused += getEntry(
    ...
    this.cumulativeStats.tokenRequests.succeeded += getEntry(
      "apiRequests.tokenRequests.succeeded",
    );
    this.cumulativeStats.tokenRequests.failed += getEntry(
      "apiRequests.tokenRequests.failed",
    );
    this.cumulativeStats.tokenRequests.refused += getEntry(
