...
export default class ConnectionsLogs extends AblyBaseCommand {
  static override args = {
    topic: Args.string({
      ...
      description:
        "Log topic to subscribe to (currently only connections-lifecycle is supported)",
    }),
  };
  ...
  async run(): Promise<void> {
    ...
    if (args.topic !== "connections-lifecycle") {
      this.error(
        `Unsupported log topic: ${args.topic}. Currently only 'connections-lifecycle' is supported.`,
      );
      return;
    }
