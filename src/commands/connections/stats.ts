...
export default class ConnectionsStats extends AblyBaseCommand {
  ...
  async runLiveStats(flags: Record<string, unknown>): Promise<void> {
    try {
      ...
      this.logCliEvent(
        flags,
        "stats",
        "liveSubscribeStarting",
        "Subscribing to live connection stats...",
      );
      ...
      const cleanup = () => {
        this.logCliEvent(
          flags,
          "stats",
          "liveCleanupInitiated",
          "Cleanup initiated for live stats",
        );
        if (this.pollInterval) {
          clearInterval(this.pollInterval);
          this.pollInterval = undefined;
      ...
      this.logCliEvent(
        ...
        "stats",
        "liveListening",
        "Now listening for live stats updates",
      );
      ...
      this.logCliEvent(
        ...
        "stats",
        "liveSetupError",
        `Error setting up live stats: ${errorMsg}`,
        { error: errorMsg },
      );
  ...
  async runOneTimeStats(flags: Record<string, unknown>): Promise<void> {
    ...
    this.logCliEvent(
      ...
      "stats",
      "oneTimeFetchRequest",
      "Fetching one-time stats with parameters",
      { params },
    );
    ...
    try {
      ...
      this.logCliEvent(
        flags,
        "stats",
        "oneTimeFetchResponse",
        `Received ${stats.length} stats records`,
        { count: stats.length, stats },
      );
      ...
      if (stats.length === 0) {
        this.logCliEvent(
          ...
          "stats",
          "noStatsAvailable",
          "No connection stats available for the requested period",
        );
        if (!this.shouldOutputJson(flags)) {
      ...
      this.logCliEvent(
        ...
        "stats",
        "oneTimeFetchError",
        `Failed to fetch one-time stats: ${errorMsg}`,
        { error: errorMsg },
      );
  ...
  private async fetchAndDisplayStats(
    ...
    try {
      ...
      this.logCliEvent(
        ...
        "stats",
        "fetchRequest",
        "Fetching stats with parameters",
        { params },
      );
      ...
      this.logCliEvent(
        ...
        "stats",
        "fetchError",
        `Failed to fetch stats: ${errorMsg}`,
        { error: errorMsg },
      );
      this.error(`Failed to fetch stats: ${errorMsg}`);
    }
  ...
  private async pollStats(
    ...
    try {
      ...
      if (flags.debug) {
        console.log(
          chalk.dim(`[${new Date().toISOString()}] Polling for new stats...`),
        );
      }
      ...
      this.logCliEvent(
        ...
        "stats",
        "pollSuccess",
        "Successfully polled and displayed stats",
      );
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : String(error);
      this.logCliEvent(
        ...
        "stats",
        "pollError",
        `Error during stats polling: ${errorMsg}`,
        { error: errorMsg },
      );
      if (flags.debug) {
