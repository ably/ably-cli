...
export default class SpacesCursorsIndex extends Command {
  ...
  async run(): Promise<void> {
    ...
    this.log("");
    this.log(
      "  ably spaces cursors set        - Set cursor position in a space",
    );
    this.log(
      "  ably spaces cursors subscribe  - Subscribe to cursor movements in a space",
    );
    this.log(
      "  ably spaces cursors get-all    - Get all current cursors in a space",
    );
    this.log("");
    this.log(
      "Run `ably spaces cursors COMMAND --help` for more information on a command.",
    );
  }
}
