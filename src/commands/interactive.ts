...
export default class Interactive extends Command {
  ...
  private displayCompletions(
    ...
    for (const item of items) {
      ...
      if (item.description) {
        console.log(
          `  ${chalk.cyan(paddedName)} -- ${chalk.gray(item.description)}`,
        );
      } else {
  ...
  private isAnonymousWebMode(): boolean {
    return (
      this.isWebCliMode() && process.env.ABLY_ANONYMOUS_USER_MODE === "true"
    );
  }
  ...
  private isCommandRestricted(commandId: string): boolean {
    ...
    if (this.isWebCliMode()) {
      ...
      if (
        WEB_CLI_RESTRICTED_COMMANDS.some((pattern) =>
      ...
      if (
        this.isAnonymousWebMode() &&
        WEB_CLI_ANONYMOUS_RESTRICTED_COMMANDS.some((pattern) =>
          this.matchesCommandPattern(commandId, pattern),
        )
      ) {
        return true;
  ...
  private updateHistorySearch() {
    ...
    const history =
      (this.rl as readline.Interface & { history?: string[] }).history || [];
  ...
  private exitHistorySearch() {
    ...
    readline.cursorTo(
      process.stdout,
      (this.rl as readline.Interface & { _prompt: { length: number } })._prompt
