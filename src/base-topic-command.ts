...
export abstract class BaseTopicCommand extends InteractiveBaseCommand {
  ...
  async run(): Promise<void> {
    ...
    if (rawArgv.length > 0) {
      ...
      if (exactCommand) {
        ...
        const subcommandIds = subcommands.map(
          (cmd) => `${this.topicName}:${cmd.id.split(" ").at(-1)}`,
        );
    ...
    if (process.env.ABLY_ANONYMOUS_USER_MODE === "true") {
      const isAnonymousRestricted = WEB_CLI_ANONYMOUS_RESTRICTED_COMMANDS.some(
        (restricted) => {
          ...
          return (
            commandId === restricted || commandId.startsWith(restricted + ":")
          );
        },
      );
  ...
  protected async getTopicCommands(): Promise<
